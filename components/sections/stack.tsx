                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border-4 border-black divide-x-0 md:divide-x-0 lg:divide-x-0">
                    {skills.map((skill, i) => {
                        ...
                        return (
                            <div
                                key={skill.id}
                                className="p-6 md:p-12 border-black border-b-4 md:border-r-4 last:border-r-0 lg:odd:border-r-4 lg:even:border-r-4 lg:[&:nth-child(4n)]:border-r-0 group hover:bg-neon transition-colors duration-300"
                            >
                                <div className="mb-6 group-hover:scale-110 transition-transform duration-300">
                                    <Icon size={48} strokeWidth={2.5} />
                                </div>
                                <h3 className="text-2xl md:text-3xl font-extrabold uppercase mb-4 leading-none tracking-tighter">{skill.name}</h3>
                                <p className="font-bold uppercase text-xs md:text-sm leading-tight opacity-70">
